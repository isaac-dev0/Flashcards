    ...
    Column(
        ...
        Row(
            ...
            Button(
                onClick = {
                    if (validateForm()) {
                        if (isEditMode && uiState.deck != null) {
                            val updatedDeck = uiState.deck!!.copy(
                                title = deckTitle.trim(),
                                description = deckDescription.trim().takeIf { it.isNotEmpty() }
                            )
                            viewModel.editDeck(updatedDeck, onDeckCreated)
                        } else {
                ...
                if (uiState.isLoading) {
                    ...
                    Text(if (isEditMode) "Update Deck" else "Create Deck")
                }
            }
        }
    }
